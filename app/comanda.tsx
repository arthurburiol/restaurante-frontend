...
export default function Comanda() {
  ...
  async function carregarComandas() {
    setLoading(true);
    try {
      const resposta = await Api.api.get("/comanda");
  ...
  const renderComandaItem = ({ item }: { item: ComandaType }) => {
    ...
    let dataAbertura = "Não informada";
      if (item.data_abertura) {
        ...
        if (!isNaN(dt.getTime())) {
          dataAbertura = dt.toLocaleString("pt-BR", {
          day: "2-digit",
          month: "2-digit",
          year: "numeric",
          hour: "2-digit",
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.buttonRow}>
        ...
        onPress={() => router.push("/comandaFechada")}>
          <Text style={styles.buttonText}>Comandas fechadas</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
